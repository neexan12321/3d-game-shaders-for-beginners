...
def build_report() -> str:
    ...
    return "\n".join(
        [
            "📊 BTCUSDT 5m candle volume",
            f"Time: {b_fut_t}",
            "",
            f"Binance Futures BTCUSDT: {_fmt_decimal(b_fut_v)} BTC",
            f"Binance Spot BTCUSDT: {_fmt_decimal(b_spot_v)} BTC",
            f"Bybit Futures BTCUSDT: {_fmt_decimal(y_fut_v)} BTC",
            f"Bybit Spot BTCUSDT: {_fmt_decimal(y_spot_v)} BTC",
        ]
    )
